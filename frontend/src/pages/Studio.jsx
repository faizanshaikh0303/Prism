/**
 * Studio — the main app page.
 *
 * Layout:
 *   ┌─────────────────────────────────────────────────────────┐
 *   │ PRISM                                       [Logout]    │  ← top bar
 *   ├───────────────┬─────────────────────────────────────────┤
 *   │               │                                         │
 *   │  Song list    │       Spatial Canvas                    │  ← main area
 *   │  (sidebar)    │                                         │
 *   │               │                                         │
 *   ├───────────────┴─────────────────────────────────────────┤
...
const POLL_INTERVAL = 4000; // ms — how often to re-check processing songs
...
export default function Studio() {
  ...
  const pollTimersRef = useRef({}); // { songId: intervalId }
  ...
  const loadLibrary = useCallback(async () => {
    ...
    try {
      ...
      // Start polling for any songs still processing
      mine.forEach((song) => {
  ...
  // ── Polling for stem-processing completion ──────────────────────────────────
      ...
      <div style={styles.main}>
          ...
          <section>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              <button
                ...
                style={{
                  ...styles.uploadBtn,
                  opacity: (uploading || mySongs.length >= 3) ? 0.4 : 1,
                }}
              >
                {uploading
                  ? `Uploading… ${uploadPct}%`
            ...
            {mySongs.length === 0 && !loadingLib && (
              <p style={styles.hint}>No songs yet. Upload an MP3, WAV or FLAC.</p>
            )}
            <ul style={styles.songList}>
              {mySongs.map((song) => (
                <SongItem
        ...
        <main style={styles.canvasArea}>
          {!currentSong ? (
            ...
            <div style={styles.empty}>
              <p style={{ opacity: 0.4, fontSize: 13 }}>Decoding audio…</p>
            </div>
          ) : (
            <>
              ...
              <div style={styles.legend}>
                {stems.map((stem) => (
                  <span
                    ...
                    style={{
                      ...styles.legendItem,
                      borderColor: STEM_COLORS[stem.stem_type] ?? '#888',
                      color: STEM_COLORS[stem.stem_type] ?? '#888',
                    }}
                  >
                    {stem.stem_type}
                  </span>
              ...
              <p style={styles.canvasTip}>
                Drag nodes · distance = volume · angle = direction · wear headphones
              </p>
            </>
          )}
        </main>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: 0 }}>
        ...
        <div style={{ minWidth: 0 }}>
          ...
          {(song.status === 'pending' || song.status === 'processing') && (
            <div style={{ ...styles.songArtist, color: '#FFEAA7' }}>
              {song.status === 'processing' ? 'Separating stems…' : 'Queued…'}
            </div>
          )}
          {song.status === 'error' && (
            <div style={{ ...styles.songArtist, color: '#FF6B6B' }}>Separation failed</div>
          )}
        </div>
      </div>
      {onDelete && (
